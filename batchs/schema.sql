-- TenaCierge Forecasting 관련 테이블 정의
-- 제공받은 운영 DB 스키마를 그대로 정리하여 추후 마이그레이션/QA에 활용한다.
...
CREATE TABLE `cleint_header` (
  ...
  `rcpt_no` VARCHAR(13) NOT NULL COMMENT '사업자번호,주민번호/숫자만',
  `rcpt_name` VARCHAR(20) NOT NULL COMMENT '영수증발행 대상의 이름',
  `rcpt_mail` VARCHAR(255) NOT NULL COMMENT '영수증 발행 받을 메일주소',
...
CREATE TABLE `etc_buildings` (
  ...
  `building_recycle` VARCHAR(10) NULL,
  `building_general` VARCHAR(10) NULL,
  `building_food` VARCHAR(10) NULL,
...
CREATE TABLE `worker_header` (
  ...
  `tier` TINYINT NOT NULL DEFAULT 3 COMMENT '1~7, 1=관리자',
  `current_score` DECIMAL(5,2) NULL COMMENT '당일 점수 평균',
