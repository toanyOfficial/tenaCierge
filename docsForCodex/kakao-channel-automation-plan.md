...
카카오와 무관하게 **모바일 웹/앱 푸시**만을 기준으로, 사용자가 보게 되는 동작을 가능한 한 구체적으로 정리했습니다. 공통 전제는 다음과 같습니다.

- **필수 전제**: 사용자가 알림 권한을 허용했고, 최신 푸시 토큰(웹 푸시라면 Service Worker 등록 + VAPID 구독 정보, 모바일 앱이라면 FCM/APNs 토큰)이 서버에 저장되어 있어야 합니다.
- **토큰 단위 행동**: 서버는 사용자 계정과 기기/브라우저 별 토큰을 매핑해 저장하고, 토큰이 만료·철회되면 삭제하거나 재구독을 받아야 합니다.
- **표시 주체**: 실제 알림 표시 여부는 OS/브라우저가 결정합니다. 서버가 200/OK를 받아도, 단말이 알림을 차단했거나 토큰이 죽어 있으면 사용자에겐 표시되지 않습니다.

### 1) 브라우저/앱(인터넷 앱)에서 푸시를 차단한 경우
- **웹(브라우저/웹뷰)**: 사용자가 사이트 알림을 차단하면, 푸시를 보내도 브라우저가 즉시 드롭합니다. 서버는 전송 성공(2xx)을 받더라도 사용자 화면에는 아무 것도 뜨지 않습니다. 다시 받게 하려면 사용자가 알림 권한을 수동으로 해제→허용해야 합니다.
- **네이티브 앱**: OS 수준에서 알림 차단 시 동일하게 표시되지 않습니다. 안드로이드는 채널 단위, iOS는 앱 전체가 차단될 수 있으니, 설정 진입 안내 배너/인앱 가이드를 별도로 둬야 합니다.

### 2) 사이트나 앱을 열어두지 않은 경우
- **웹 푸시**: 탭을 닫거나 백그라운드여도 Service Worker 등록 + 권한 허용 상태면 브라우저가 백그라운드에서 알림을 띄웁니다. 다만 크롬·사파리 모바일은 장기간 미사용 시 Service Worker가 정리될 수 있어 토큰이 만료될 수 있습니다.
- **모바일 앱**: 앱이 종료되었더라도 FCM/APNs 토큰이 유효하고 OS가 네트워크에 연결되어 있으면 시스템 트레이에 알림이 도착합니다. 사용자가 한 번도 권한을 허용하지 않았거나 알림을 꺼두면 표시되지 않습니다.

### 3) 오래 전에 띄워둔 탭/앱(장기 미접속)인 경우
- **웹**: 며칠~수주 이상 미방문 시 브라우저가 웹 푸시 구독을 만료시키거나(410/404 응답) 백그라운드 실행을 제한합니다. 이런 응답을 감지해 해당 토큰을 삭제하고, 다음 방문 시 재구독을 요청해야 재활성화됩니다.
- **앱**: 장기간 미사용 시 OS가 앱 백그라운드를 제한하거나 토큰을 회수할 수 있습니다(특히 iOS). 전송 실패 로그가 반복되면 토큰을 폐기하고, 앱 재실행 시 신규 토큰을 받아야 합니다.

### 4) 로그인이 해제된 상태인 경우
- **토큰 정리 원칙**: 로그아웃 시 계정과 토큰의 연결을 끊고, 정책에 따라 토큰 자체를 삭제하거나 비활성화해야 합니다. 그렇지 않으면 타인이 동일 단말을 사용할 때 이전 계정 알림이 표시될 수 있습니다.
- **발송 시나리오**: 서버가 토큰을 남겨둔 상태로 푸시를 보내면 기술적으로는 알림이 표시될 수 있으나, 개인정보 노출 위험이 있으므로 "로그인 유지 + 토큰 유효"가 아닌 토큰은 발송 대상에서 제외하는 것이 안전합니다.

### 5) 오프라인/불안정 네트워크
- **일시 오프라인**: OS/브라우저는 짧은 시간 재시도 후 포기하거나 지연 전달합니다. 장시간 오프라인이면 알림이 폐기될 수 있으므로, 중요한 메시지는 사용자가 복귀했을 때 인앱 알림센터/Inbox에서 다시 볼 수 있게 저장해 두는 편이 좋습니다.

### 6) 알림 표시/핸들러 세부사항
- **표시 시점**: 웹은 Service Worker의 `push` 이벤트, 앱은 FCM/APNs를 통해 수신 후 시스템 트레이에 표시됩니다. `data-only` 페이로드(소리/배너 없이 데이터만)는 앱/브라우저가 활성이 아니면 무시될 수 있으니, 실제 표시를 원하면 알림 페이로드를 포함해야 합니다.
- **페이로드 제한**: FCM/APNs는 크기 제한(수 KB)과 필드 형식이 있습니다. 너무 큰 데이터는 서버에서 축약하거나 링크만 전달하고, 상세 내용은 앱/웹이 열렸을 때 API 호출로 조회하도록 설계합니다.

### 7) 실무 권장사항 (푸시 단독 기준)
- 권한 상태, 토큰 만료(410/404), 전송 실패 로그를 모니터링하고, 토큰 정리 + 재구독 유도를 자동화합니다.
- 계정 로그아웃/탈퇴/기기 변경 시 토큰을 즉시 해제해 잘못된 사용자에게 알림이 뜨지 않도록 합니다.
- 중요한 알림은 인앱 알림센터/히스토리에 별도로 저장해, 푸시를 놓쳤을 때도 사용자가 확인할 수 있게 합니다.
