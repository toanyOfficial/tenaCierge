...
DDL_FILE = Path(__file__).with_name("schema.ddl.sql")
...
def fetch_table_creates(conn) -> List[Tuple[str, str]]:
    """Return table name and CREATE TABLE DDL for all base tables in the DB."""
    ...
    cursor.execute("SHOW FULL TABLES IN `%s` WHERE Table_type = 'BASE TABLE'" % db_name)
    ...
    for table in tables:
        ...
        if not result or len(result) < 2:
            continue
        ddl_pairs.append((table, result[1]))
...
def write_ddl(table_creates: List[Tuple[str, str]]) -> None:
    """Write CREATE TABLE statements to a single SQL file."""
    ...
    for table_name, ddl in table_creates:
...
def main():
    ...
    try:
        ...
        table_creates = fetch_table_creates(conn)
    finally:
        conn.close()
