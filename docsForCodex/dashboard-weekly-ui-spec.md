...
- **디자인**: 섹터별 카드 3개를 **1row × 3col** 그리드로 배치해 높이 100%를 차지한다(물량이 없어도 1×3 유지). 카드에는 총 물량, 건물별/체크아웃 시각별 건수, Role별 apply 슬롯(슬롯 번호 기준, 팀 호칭 금지)을 표시하며, 미배정 슬롯 텍스트는 **red**로 노출한다.
- **슬롯 표기**: apply 슬롯은 사람 단위이며 `슬롯 N`으로 지칭한다. Role 구분은 필수로 함께 노출한다.
