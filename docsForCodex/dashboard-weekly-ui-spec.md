...
### D1-compact
- **역할**: D1 정보를 섹터별 총 건수와 역할별 apply 슬롯 현황만 간결하게 보여주는 보조 뷰.
- **데이터·갱신**: D1-dominant와 동일 스냅샷을 공유한다(정시 리프레시 문구 없음).
- **디자인**: 섹터 카드에는 총 예정 건수와 Role 1=클리너, Role 2=버틀러 슬롯 배치를 배지로 노출한다. 건물/체크아웃 표기나 별도 라벨(배치 모니터링 등)은 사용하지 않는다.
