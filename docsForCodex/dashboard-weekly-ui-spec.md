...
- **디자인**: 섹터별 카드 3개를 **1row × 3col** 그리드로 배치해 높이 100%를 차지한다(물량이 없어도 1×3 유지). 카드에는 총 물량과 건물별 박스 안의 체크아웃 시각별 건수, Role별 apply 슬롯(슬롯 번호 기준, 팀 호칭 금지)을 표시하며, 미배정 슬롯 텍스트는 **red**로 노출한다. 헤더·배지 색상을 강화해 가시성을 높이며, 건물/체크아웃 영역과 역할별 슬롯 영역은 색상·분리 바를 달리해 서로 구분한다.
