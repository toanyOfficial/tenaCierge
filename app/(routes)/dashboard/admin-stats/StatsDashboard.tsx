...
function buildSectorPalette(sectorCode: number | null, count: number) {
  const fallback = { start: '#cbd5e1', end: '#475569' };
  const sectorStops: Record<number, { start: string; end: string }> = {
    1: { start: '#bae6fd', end: '#0b4f71' },
    2: { start: '#fde68a', end: '#78350f' },
    3: { start: '#bbf7d0', end: '#166534' }
  };

  const { start, end } = sectorStops[sectorCode ?? 0] ?? fallback;
...
export default function StatsDashboard({
  ...
  const weekdayLineColorMap = useMemo(() => {
    const grouped = new Map<number | null, WeekdaySeriesMeta[]>();

    weekdayStats.buildings.forEach((meta) => {
      const sector = meta.sectorCode ?? null;
      if (!grouped.has(sector)) {
        grouped.set(sector, []);
      }
      grouped.get(sector)!.push(meta);
    });

    const colorMap = new Map<string, string>();

    grouped.forEach((metas, sector) => {
      const sorted = [...metas].sort((a, b) => (b.totalCount ?? 0) - (a.totalCount ?? 0));
      const palette = buildSectorPalette(sector, sorted.length);

      sorted.forEach((meta, index) => {
        const paletteIndex = Math.min(index, palette.length - 1);
        colorMap.set(meta.key, palette[paletteIndex]);
      });
    });
  ...
  const WeekdayLegend = useMemo(
      ...
      function LegendContent() {
        return (
          <div className={styles.chartLegend} aria-label="범례">
            ...
            {weekdayStats.buildings.map((meta) => {
              const color = weekdayLineColorMap.get(meta.key) ?? '#38bdf8';
              return (
    ...
    [weekdayLineColorMap, weekdayStats.buildings]
  );
