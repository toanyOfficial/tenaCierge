...
export async function fetchWeekdayStats(): Promise<{
  ...
  const buildingMeta: WeekdaySeriesMeta[] = buildingIds.map((id) => {
    ...
    const sectorCode = match?.sectorCode === null || match?.sectorCode === undefined ? null : Number(match.sectorCode);
    const totals = buildingTotals.get(id);
    const totalCount = totals ? Array.from(totals.values()).reduce((acc, count) => acc + count, 0) : 0;
    return { key: makeBuildingKey(id), label, sectorCode, totalCount };
  });
