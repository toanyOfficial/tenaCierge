...
export default function StatsTableDashboard({ snapshot }: { snapshot: StatsTableSnapshot }) {
  ...
  const monthLegends = [...monthlyBarSeries, ...monthlyLineSeries].map((series) => ({
    key: series.key,
    label: series.plan,
    color: resolveColor(series.key, legendColors),
    type: series.plan === '정액제' ? 'line' : 'bar'
  }));
          ...
          <section className={styles.quadrant}>
            <div className={styles.monthChart}>
              <div className={styles.monthChartArea}>
                    ...
                    return (
                      <g key={series.key}>
                        ...
                        {series.values.map((value, idx) => {
                          ...
                          return <circle key={`${series.key}-dot-${idx}`} cx={x} cy={y} r={2.6} fill={color} stroke="none" />;
                        })}
                      </g>
                    );
                  })}
                ...
                <div className={styles.monthLegendPanel}>
                  {monthLegends.map((legend) => (
                    <div key={legend.key} className={styles.monthLegendRow}>
                      <span
                        className={styles.legendSquare}
                        style={{ backgroundColor: legend.color, borderRadius: legend.type === 'line' ? '999px' : '4px' }}
                      />
                      <span className={styles.legendText}>{legend.label}</span>
                    </div>
                  ))}
                </div>
              </div>
