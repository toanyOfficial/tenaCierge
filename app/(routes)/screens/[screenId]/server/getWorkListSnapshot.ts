...
export async function getWorkListSnapshot(
  ...
  try {
    ...
    const baseWhere = and(eq(workHeader.date, targetDateSql), eq(workHeader.cancelYn, false));
    ...
    if (isAdmin || isButler) {
      ...
      if (!client) {
        ...
        rows = await baseQueryBuilder
          .where(and(baseWhere, eq(clientRooms.clientId, client.id)))
          .limit(1000);
      }
    } else if (isCleaner) {
      if (!worker) {
    ...
    const workGlobalCompletedRooms = await fetchWorkGlobalCompletions(
      activeWorkGlobals.map((item) => item.id)
    );
        ...
        workGlobals:
          activeWorkGlobals.length && work.roomId
            ? activeWorkGlobals.map((header) => ({
                ...
                title: header.title,
                dscpt: header.dscpt,
                completed: workGlobalCompletedRooms.get(header.id)?.has(work.roomId) ?? false
              }))
            : []
      }))
...
async function fetchWorkGlobalCompletions(headerIds: number[]) {
  ...
  const rows = await db
    ...
    .where(inArray(workGlobalDetail.workGlobalId, headerIds));

  rows.forEach((row) => {
    const key = Number(row.workGlobalId);
